...
contract("RegulatedToken", (accounts) => {
  ...
  beforeEach(async () => {
    instance = await RegulatedToken.new("Rewards", "RWD", 18, regulatorService.address);
  });
  ...
  it('should allow minter to mint new coins', async () => {
    ...
    await instance.mint(creditAccount, amountToCredit, {from: mintingAccount});
  ...
  it('should not allow non-minter to mint new coins', async () => {
    const creditAccount = accounts[1];
    await expect(
      instance.mint(creditAccount, 1000, {from: creditAccount})
    ).to.be.rejected;
  });
});